"""Canonical VOC taxonomy and deterministic operations policy for v0.1."""
...
TAXONOMY_DETAILS: dict[str, tuple[TaxonomyItem, ...]] = {
    "배송": (
...
TAXONOMY: dict[str, tuple[str, ...]] = {
    category: tuple(item["subcategory"] for item in items)
    for category, items in TAXONOMY_DETAILS.items()
}
ALL_SUBCATEGORIES = [item for items in TAXONOMY.values() for item in items]
...
HUMAN_REVIEW_SUBCATEGORIES = frozenset(
    {
        "개인정보/보안",
        "중복 결제",
        "배송완료 미수령",
...
HUMAN_REVIEW_RULES = (
    ...
    "환불 지연·부분 환불·환불 금액처럼 금전 확인이 필요한 경우",
